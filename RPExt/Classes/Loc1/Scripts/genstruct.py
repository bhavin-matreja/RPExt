...
def main():
  ...
  with open(os.path.join(CURRENT_DIR,"settings.json")) as setting_file:
    ...
    GEN_STRUCT_TABLE_NAME = "Localizable"
    ...
    for dirname, dirnames, filenames in os.walk(os.path.join(CURRENT_DIR, IN_PATH)):
      ...
      fwrite.write("import Foundation \n\n")
      fwrite.write("public class {0} {{\n".format(GEN_STRUCT_STRUCT_NAME))
      # for each .csv files
      for f in filenames:
        ...
        with open(fullpath, 'rb') as csvfile:
          ...
          for row in iterrows:
            ...
            # 2nd Option
            fwrite.write('\t\tpublic static func {0}(params: [CVarArg]) -> String {{\n'.format(full_key))
            fwrite.write('\t\t\treturn String(format: NSLocalizedString("{0}", tableName: "{1}", bundle: Bundle(for: {2}.self), comment: "{0}"), params) '.format(full_key, GEN_STRUCT_TABLE_NAME, GEN_STRUCT_STRUCT_NAME) + '\n')

            fwrite.write('\t\t}\n')
            fwrite.write('\t\tpublic static let {0} = NSLocalizedString("{0}", tableName: "{1}", bundle: Bundle(for: {2}.self), comment: "{0}")\n'.format(full_key, GEN_STRUCT_TABLE_NAME, GEN_STRUCT_STRUCT_NAME) + '\n\n')
