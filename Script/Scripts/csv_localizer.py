...
def main():
  ...
  with open(os.path.join(CURRENT_DIR, "settings.json")) as setting_file:
    ...
    OUT_PATH_ANDROID = setting["OUT_PATH_ANDROID"]
    ...
    print "Out path Android: {0}".format(OUT_PATH_ANDROID)
    ...
    if PLATFORM == "ios":
      ...
      localize_android(BASE_STRINGS_PATH, IN_PATH, OUT_PATH_ANDROID, LANG_KEYS)
...
def localize_android(BASE_PATH, IN_PATH, OUT_PATH_ANDROID, LANG_KEYS):
  base_out_dir = os.path.join(BASE_PATH, OUT_PATH_ANDROID)
  # top most
  if not os.path.exists(base_out_dir):
      os.makedirs(base_out_dir)
    ...
    for f in filenames:
      ...
      with open(fullpath, 'rb') as csvfile:
        ...
        for row in iterrows:
          ...
          [fwrite.write('\t<string name="{domain}_{key}">{lang}</string>\n'.format(domain=filename, key=row_key, lang=escape(row_values[idx]))) for idx, fwrite in enumerate(allwrites)]
    [fwrite.write('</resources>') for fwrite in allwrites]
  [fwrite.close() for fwrite in allwrites]
